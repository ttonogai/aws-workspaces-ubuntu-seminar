...
if [[ -z "$performance_bundles" ]]; then
    ...
    # 日本語版を優先
    japanese_performance=$(echo "$performance_bundles" | jq -r 'select(.Name | contains("Japanese"))')
    ...
    if [[ -n "$japanese_performance" ]]; then
        ...
        recommended_bundle_id=$(echo "$performance_bundles" | jq -r '.BundleId')
        recommended_bundle_name=$(echo "$performance_bundles" | jq -r '.Name')
        print_color "green" "✓ 推奨Bundle ID: $recommended_bundle_id"
        print_color "green" "✓ Bundle名: $recommended_bundle_name"
    fi
